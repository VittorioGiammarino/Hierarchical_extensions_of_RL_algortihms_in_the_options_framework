...
class H_UATRPO:
    def __init__(self, state_dim, action_dim, action_space_cardinality, max_action, min_action, option_dim, termination_dim,
                 num_steps_per_rollout=5000, pi_b_freq=1, pi_hi_freq=1, l_rate_pi_lo=3e-4, l_rate_pi_hi=3e-4 , l_rate_pi_b=3e-4, l_rate_critic=3e-4, 
                 gae_gamma = 0.99, gae_lambda = 0.99, epsilon = 0.03, conj_grad_damping=0.1, 
                 random_projections_pi_hi = 40, random_projections_pi_lo = 200, random_projections_pi_b = 100, 
                 beta = 0.9, alpha = 0.05, c=6e-4, lambda_ = 1e-3, eta = 1e-7, min_batch_size = 32):
        ...
        self.Y_F_old_pi_lo = [[None]*1 for _ in range(option_dim)]
        self.Y_S_old_pi_lo = [[None]*1 for _ in range(option_dim)]
        self.d_pi_lo = [[None]*1 for _ in range(option_dim)]
        self.random_projections_pi_lo = random_projections_pi_lo
        self.Random_matrix_pi_lo = [[None]*1 for _ in range(option_dim)]
        
        for option in range(option_dim):
            self.d_pi_lo[option] = sum(p.numel() for p in self.pi_lo[option].parameters() if p.requires_grad)
            self.Random_matrix_pi_lo[option] = torch.randn((self.d_pi_lo[option], self.random_projections_pi_lo), dtype=torch.float32)
            self.Y_F_old_pi_lo[option] = torch.zeros_like(self.Random_matrix_pi_lo[option])
            self.Y_S_old_pi_lo[option] = torch.zeros_like(self.Random_matrix_pi_lo[option])
            
        self.Q_pi_lo = [[None]*1 for _ in range(option_dim)]
    ...
    def GAE(self, env):
        ...
        while step < self.num_steps_per_rollout: 
            ...
            initial_option = 0
            initial_b = 1
            option = H_UATRPO.select_option(self, state, initial_b, initial_option)

            while not done and step < self.num_steps_per_rollout:            
                action = H_UATRPO.select_action(self, state, option)
                ...
                termination = H_UATRPO.select_termination(self, state, option)
                next_option = H_UATRPO.select_option(self, state, termination, option)
                option = next_option
    ...
    def rescale_and_linesearch(self, g, eta_v, Fv, Sv, L, Rn2, kld, old_params, option, max_iter=10, success_ratio=0.1):
        H_UATRPO.set_params(self.pi_lo[option], old_params)
        L_old = L().detach()
        max_kl = self.epsilon
        ...
        for _ in range(max_iter):
            ...
            H_UATRPO.set_params(self.pi_lo[option], new_params)
            kld_new = kld().detach()
        ...
        print(f"The line search for pi_lo option {option} failed!")
    ...
    def rescale_and_linesearch_termination(self, g, s, Hs, L, kld, old_params, option, max_iter=10, success_ratio=0.1):
        H_UATRPO.set_params(self.pi_b[option], old_params)
        L_old = L().detach()
        max_kl = self.epsilon
        ...
        for _ in range(max_iter):
            ...
            H_UATRPO.set_params(self.pi_b[option], new_params)
            kld_new = kld().detach()
    ...
    def rescale_and_linesearch_pi_hi(self, g, s, Hs, L, kld, old_params, max_iter=10, success_ratio=0.1):
        H_UATRPO.set_params(self.pi_hi, old_params)
        L_old = L().detach()
        max_kl = self.epsilon
        ...
        for _ in range(max_iter):                   
            ...
            H_UATRPO.set_params(self.pi_hi, new_params)
            kld_new = kld().detach()
    ...
    def train(self, Entropy=False):
        ...
        for option in range(self.option_dim):
            ...
            if len(states_op)>=self.min_batch_size:
                ...
                old_params = H_UATRPO.get_flat_params(self.value_function).detach()
                ...
                gradient_constraint = H_UATRPO.get_flat_grads(constraint(), self.value_function)
                
                def Hv(v):
                    hessian_v = H_UATRPO.get_flat_grads(torch.dot(gradient_constraint, v), self.value_function).detach()
                    return hessian_v
               
                gradient = H_UATRPO.get_flat_grads(((-1)*(self.value_function(states_op, option_vector).squeeze() - returns_op)**2).mean(), self.value_function).detach()
                s = H_UATRPO.conjugate_gradient(Hv, gradient).detach()
                Hessian_s = Hv(s).detach()
                alpha = torch.sqrt(2*self.epsilon/torch.dot(s,Hessian_s))
                new_params = old_params + alpha*s
                H_UATRPO.set_params(self.value_function, new_params)            
               
                self.pi_lo[option].train()
                old_params = H_UATRPO.get_flat_params(self.pi_lo[option]).detach()
                ...
                grad_kld_old_param = H_UATRPO.get_flat_grads(kld(), self.pi_lo[option])
           
                def Hv(v):
                    hessian_v = H_UATRPO.get_flat_grads(torch.dot(grad_kld_old_param, v), self.pi_lo[option]).detach()
                    return hessian_v + self.conj_grad_damping*v
            
                gradient = H_UATRPO.get_flat_grads(L(), self.pi_lo[option]).detach()
                ...
                Y_F_cur = torch.zeros_like(self.Random_matrix_pi_lo[option])
                for i in range(self.random_projections_pi_lo):
                    Y_F_cur[:,i] = Hv(self.Random_matrix_pi_lo[option][:,i])
                    
                Y_F_new = self.beta*self.Y_F_old_pi_lo[option] + (1-self.beta)*Y_F_cur
                Y_F = Y_F_new/(1-self.beta**(self.beta_t+1))
                
                Y_S_cur = torch.zeros_like(self.Random_matrix_pi_lo[option])
                for i in range(self.random_projections_pi_lo):
                    Y_S_cur[:,i] = Sigmav(self.Random_matrix_pi_lo[option][:,i])
                    
                Y_S_new = self.beta*self.Y_S_old_pi_lo[option] + (1-self.beta)*Y_S_cur
                Y_S = Y_S_new/(1-self.beta**(self.beta_t+1))            
                
                Rn2 = (self.d_pi_lo[option] + 2 * np.sqrt(self.d_pi_lo[option]*np.log(1/self.alpha)) + 2*np.log(1/self.alpha)) / self.Total_trajs
                ...
                try:
                    ...
                    Q = Q[:,:Y_rank]
                    self.Q_pi_lo[option] = torch.FloatTensor(Q)      
                except:
                    print('error due to a ill-defined gradient or hessian, trying with a larger number of steps per iteration could solve the problem')
                
                def M(x):
                    return Hv(x) + self.trade_off*Rn2*Sigmav(x)+self.conj_grad_damping*x
                
                if self.beta == 0:
                    MQ = torch.zeros_like(self.Q_pi_lo[option])
                    for i in range(Y_rank):
                        MQ[:,i] = M(self.Q_pi_lo[option][:,i])  
                    M_proj = torch.matmul(torch.transpose(self.Q_pi_lo[option],0,1),MQ)
                else:
                    lstsq_goal = torch.matmul(torch.transpose(Y+self.conj_grad_damping*self.Random_matrix_pi_lo[option],0,1),self.Q_pi_lo[option])
                    lstsq_input = torch.matmul(torch.transpose(self.Random_matrix_pi_lo[option],0,1),self.Q_pi_lo[option])
                ...
                U = torch.matmul(self.Q_pi_lo[option],V)
                ...
                new_params, Failed = H_UATRPO.rescale_and_linesearch(self, gradient, eta_v_flat, _, _, L, Rn2, kld, old_params, option)
                ...
                if Entropy and not Failed:
                    if self.action_space == "Discrete":
                        ...
                        gradient_discounted_casual_entropy = H_UATRPO.get_flat_grads(discounted_casual_entropy, self.pi_lo[option])
                ...
                self.Y_F_old_pi_lo[option] = Y_F_new
                self.Y_S_old_pi_lo[option] = Y_S_new
            ...
            if self.Total_iter % self.pi_b_freq == 0:
                ...
                if len(states_op_termination_true)>=self.min_batch_size:
                    ...
                    old_params_termination = H_UATRPO.get_flat_params(self.pi_b[option]).detach() 
                    ...
                    grad_kld_termination_old_param = H_UATRPO.get_flat_grads(kld_termination(), self.pi_b[option])
                
                    def Hv(v):
                        hessian_v = H_UATRPO.get_flat_grads(torch.dot(grad_kld_termination_old_param, v), self.pi_b[option]).detach()
                        return hessian_v + self.conj_grad_damping*v
                
                    gradient_termination = H_UATRPO.get_flat_grads(L_termination(), self.pi_b[option]).detach()
                    s = H_UATRPO.conjugate_gradient(Hv, gradient_termination).detach()
                    Hs = Hv(s).detach()
                    new_params_termination, Failed = H_UATRPO.rescale_and_linesearch_termination(self, gradient_termination, s, Hs, L_termination, kld_termination, old_params_termination, option)
                    ...
                    if Entropy and not Failed:
                        ...
                        gradient_discounted_causal_entropy_termination = H_UATRPO.get_flat_grads(discounted_causal_entropy_termination, self.pi_b[option])
                        new_params_termination += self.lambda_*gradient_discounted_causal_entropy_termination
    
                    H_UATRPO.set_params(self.pi_b[option], new_params_termination)    
        ...
        if self.Total_iter % self.pi_hi_freq == 0:
            ...
            if len(states_termination_true)>=self.min_batch_size:
                ...
                self.pi_hi.train()
                old_params_pi_hi = H_UATRPO.get_flat_params(self.pi_hi).detach() 
                ...
                grad_kld_pi_hi_old_param = H_UATRPO.get_flat_grads(kld_pi_hi(), self.pi_hi)
            
                def Hv(v):
                    hessian_v = H_UATRPO.get_flat_grads(torch.dot(grad_kld_pi_hi_old_param, v), self.pi_hi).detach()
                    return hessian_v + self.conj_grad_damping*v
            
                gradient_pi_hi = H_UATRPO.get_flat_grads(L_pi_hi(), self.pi_hi).detach()
                s = H_UATRPO.conjugate_gradient(Hv, gradient_pi_hi).detach()
                Hs = Hv(s).detach()
                new_params_pi_hi, Failed = H_UATRPO.rescale_and_linesearch_pi_hi(self, gradient_pi_hi, s, Hs, L_pi_hi, kld_pi_hi, old_params_pi_hi)
            
                if Entropy and not Failed:
                    _, entropy_log_prob = self.pi_hi.sample_log(states_termination_true, options_termination_true)
                    discounted_casual_entropy = ((-1)*gammas_termination_true*entropy_log_prob).mean()
                    gradient_discounted_casual_entropy = H_UATRPO.get_flat_grads(discounted_casual_entropy, self.pi_hi)
                    new_params_pi_hi += self.lambda_*gradient_discounted_casual_entropy
                    
                H_UATRPO.set_params(self.pi_hi, new_params_pi_hi)
